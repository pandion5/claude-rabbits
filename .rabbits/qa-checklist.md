...
- release.sh가 커밋 트레일러를 환경변수로 받는가 — 세션 URL을 스크립트에 박으면 다음 릴리스부터 남의 세션 URL이 이력에 남는다 (검증: `RABBITS_CLAUDE_SESSION=https://example/zz sh scripts/release.sh --dry-run 9.9.9 "t" README.md | grep -c 'Claude-Session: https://example/zz'` = 1, 같은 명령을 변수 없이 돌리면 `grep -c 'Claude-Session'` = 0. 드라이런은 파일을 쓰지 않는다)
- 트레일러 두 줄이 한 `-m` 안에 들어가는가 — `-m`을 두 번 쓰면 사이에 빈 줄이 끼어 git 트레일러 블록으로 인식되지 않는다 (검증: `grep -c 'git commit -m "\$SUBJECT" -m "\$TRAILERS"' scripts/release.sh` = 1)

## 의도 기록

- 단계 0이 의도 기록을 5절 서식으로 지시하는가 — 절 제목 5종이 모두 단계 0 구간에 있어야 한다 (검증: `awk '/^## 단계 0/{f=1;next} f&&/^## /{exit} f' skills/run/SKILL.md | grep -cE '^  ## (문제|제안 결과|영향 범위|제약|미해결 질문)$'` = 5. **앵커 `^  `와 `$`를 빼면 6이 된다** — 같은 구간 산문이 `` `## 제약` ``·`` `## 미해결 질문` ``을 인라인 코드로 다시 언급해 줄 단위로 한 번 더 세어진다. 서식 블록 안의 절 제목은 들여쓰기돼 있어 `^## ` 구간 종료에도 걸리지 않는다)
