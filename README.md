...
[graft](https://github.com/trailhq/Graft)가 설치돼 있으면 단계 1에서 좌표 정찰에 쓴다. 없으면 기존 방식
그대로 동작하므로 설치는 선택이다. 대장이 대상 리포에 `graft build`를 한 번 돌려 구조 그래프를 만들고
(`graft/`는 graft가 `.gitignore`에 자동 추가한다), `callers`·`skeleton`·`ask`로 뽑은 `파일:줄`을 컨텍스트
팩에 넣는다. LLM 요약 패스(`--deep`)는 쓰지 않는다. 소스를 외부 모델로 보내는 옵션이라 사내 코드에서는
별도 판단이 필요하다.
