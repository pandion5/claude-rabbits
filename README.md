...
| 2 | 편성 | 아키타입 100종(코어 17 + 확장 83, 분야별 지연 로드) + 전문 팀 프리셋 5종(테크·법무·보안·서치·QA) + 모델·제한시간 배정 |
...
### 아키타입 (100종 = 코어 17 + 확장 83)

**코어 17종** — 매 런 로드되는 기본 역할:
...
**확장 83종** — 11개 분야로 `skills/run/archetypes-ext/`에 상비, 대장이 서브작업에
...
├── skills/run/
│   ├── SKILL.md          # 6단계 프로토콜
│   ├── archetypes.md     # 코어 17종 + 공통 규칙 + 확장 카탈로그 인덱스
│   ├── archetypes-ext/   # 확장 아키타입 83종 (11분야, 지연 로드)
