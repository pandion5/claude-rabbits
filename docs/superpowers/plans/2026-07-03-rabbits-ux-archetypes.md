...
### Task 2: README.md — 17종 반영 + 아키타입 목록 표 신설

(사용자 추가 요청 2026-07-03: README에 아키타입을 명시할 것 — 목록 표 신설)

**Files:**
- Modify: `README.md` (동작 방식 표 단계 2 행 34행 부근, 전문 팀 표 아래, 파일 구조 63행 부근)

**Interfaces:**
- Consumes: Task 1의 수치(17종)와 아키타입 명칭 전체.
...
`### 전문 팀 (프리셋)` 섹션의 마지막 줄(`대장이 작업 성격을 보고 자동 투입한다 — 별도 명령 없음.`) **바로 아래**에 삽입:
...
| 4 | 테스터 | 테스트 작성·실행·보고 |
| 5 | 기획가 | 서브도메인 분해·설계 |
| 6 | 테크 검토가 | 기술 가능성 판정 (가능/조건부/불가) |
| 7 | 라이선스 감사가 | 라이선스·상업적 사용 검토 (법률 자문 아님) |
| 8 | 보안 감사가 | 취약점 스캔 (방어 전용) |
| 9 | 서처 | 웹+코드 멀티모달 자료 조달 |
| 10 | 통합가 | 여러 산출물 병합·최종 조립 |
| 11 | 디버거 | 재현→근본원인→최소 수정 |
| 12 | 검증가 | 산출물 실제 실행·동작 관찰 (E2E) |
| 13 | 문서가 | 대상 독자 맞춤 문서 작성 |
| 14 | 마이그레이터 | 반복 패턴 일괄 변환 (전수성 의무) |
| 15 | 여정 점검가 | 사용자 시나리오 주행 — 막힘 지점 발견 |
| 16 | UX 분석가 | UX 전문 기준 평가 (원칙 인용 의무) |
| 17 | 디자인 감사가 | 원본 대비 픽셀 단위 시각 정합 대조 |
```

- [ ] **Step 4: 검증**

Run: `Grep pattern="아키타입 17종|템플릿 17종|### 아키타입 \(17종\)" path="README.md" output_mode="count"`
Expected: 3개 패턴 각 1회.

Run: `Grep pattern="14종" path="README.md" output_mode="count"`
Expected: 0 (옛 수치 잔존 없음).

Run: `Grep pattern="^\| 1[0-7] \|" path="README.md" output_mode="count"`
Expected: 8 (표에 10~17행 존재 — 전체 17행 표의 하한 확인).
...
git add README.md
git commit -m "$(cat <<'EOF'
docs: README 아키타입 17종 반영 + 목록 표 신설

수치 갱신(단계 2 행·파일 구조)에 더해 아키타입 17종 전체를 표로 명시
(사용자 요청). 팀 표 아래에 배치.
