...
- 공통 슬롯 6종: `{{컨텍스트}}`(배경·상류 산출물 요약) · `{{작업}}`(정확한 지시) ·
  `{{제약}}`(추가 제약) · `{{완료조건}}`(검증 가능한 체크리스트) · `{{출력형식}}`(추가 반환 계약) ·
  `{{제외범위}}`(**선택** — 대장이 필요 시 채우는 사전 스코프 계약: 이 작업에서 명시적으로
  하지 않을 것 — 인접 개선·리팩토링·추가 기능 등)
...
- 코드 산출물 표준: 2칸 들여쓰기, 한국어 주석, 리포 관례 준수.
  단, `{{컨텍스트}}`에 리포별 설정(`.rabbits/config.md`)이 실려 오면 **그 값이 이 기본값보다 우선**한다.
...
- **제외범위 준수**: `{{제외범위}}`에 적힌 항목은 손대지 말 것 — 고치고 싶어도 그대로 둔다.
  꼭 필요하다고 판단되면 `notes`에 1줄 제안만 남기고 작업은 하지 않는다(실행은 대장이 결정).
- **durability(썩지 않는 서술)**: 산출물·완료조건은 **행동·상태 기준**으로 쓰고 파일 경로·줄 번호에
  의존하지 말 것(코드가 바뀌면 썩는다). 위치는 `evidence`의 근거로만 쓴다.
...
가설을 세우기 **전에** 관찰 신호(피드백 루프)를 먼저 만든다 — 위에서부터 가능한
첫 수단을 채택: ① 실패하는 자동 테스트 ② 최소 재현 스크립트·명령 ③ 실행 로그·계측
출력(임시 로그 추가) ④ HTTP·CLI 응답 스냅샷 ⑤ 헤드리스 브라우저 관찰 ⑥ 수동 재현
절차 문서화. 여섯 중 하나도 못 만들면 가설 단계로 진행 금지 — `outcome: BLOCKED` +
`need:`에 어떤 신호가 필요한지 명시하라. 진단용으로 넣은 임시 로그·디버그 코드는
수정 완료 시 전부 제거하고, 제거 여부를 `self_check` 항목으로 넣는다.
...
"수정 후 원 재현 절차 미재현 확인"과 "임시 계측·디버그 코드 전량 제거" 항목 필수.
{{출력형식}}
잡담·과정 설명은 최종 메시지에 남겨도 되지만, 대장은 공통 규칙의 `rabbits-result` 블록만 읽는다. 반드시 최종 메시지 끝에 그 블록을 붙여라. 너의 결과 블록이 곧 산출물이다.
```
